...
from model import BPRegression,BPClassification,RBFGradClassification,RBFGradRegression,RBFBPClassification,RBFBPRegression
...
def train(model,loadedData,args):
    ...
    for k in range(args.epoches):
        ...
        for j in range(length):
            ...
            model.sgd_backward(input,target,args.learning_rate,args.lambd)
...
def main():
    ...
    parser.add_argument("--model",default="BP",help="Defination of the model",type=str)
    parser.add_argument("--act_func",default="relu",help="Defination of the kernel activate function.",type=str)
    ...
    # preparing model
    if args.model == "BP" and (args.type == "Red-Wine" or args.type == "White-Wine"):
        model = BPClassification(in_dim=11,hid_dim=args.hid_dim,n_class=10,act_name=args.act_func,name=rq)
    ...
    elif args.model == "RBFBP" and (args.type == "Red-Wine" or args.type == "White-Wine"):
        model = RBFBPClassification(in_dim=11,hid_dim=args.hid_dim,n_class=10,act_name=args.act_func,name=rq)
    elif args.model == "BP" and args.type == "Forecast":
        model = BPRegression(in_dim=21,hid_dim=args.hid_dim,out_dim=2,act_name=args.act_func,name=rq)

...
class GA:
    def __init__(self,popsize,iterations,pc=0.6,pc_num=0.4,pm = 0.05,pr =0.05,ggap=0.1):
    ...
    def solve(self,citys_mat):
        ...
        # 初始化种群，生成随机的染色体
        pop_store = np.zeros((self.popsize,citys_num),dtype=np.int)
        for k in range(self.popsize):
            pop_store[k] = np.random.permutation(citys_num)
        ...
        for _iter in range(self.iterations):
            # 用于保存当前的个体适应度的值
            fit_current = np.zeros((self.popsize,))
            # 计算种群的适应度
            for k in range(self.popsize):
                fit_val = self.fitness(pop_store[k],citys_distance)
                fit_current[k] = fit_val
            # 进行自然选择操作,产生新的种群
            pop_store_new = np.zeros((self.popsize, citys_num), dtype=np.int)
            # 方法一：使用轮盘赌方法选择个体并产生新的种群

            fit_sum = np.sum(fit_current)
            # 用于保存当前染色体被复制的概率
            c_prob_current = fit_current/fit_sum
            # 用于计算当前染色体被复制的累积概率值
            cum_prob_current = np.cumsum(c_prob_current)
            for k in range(self.popsize):
                probability = np.random.random()
                index = np.where(cum_prob_current > probability)[0][0]
                pop_store_new[k] = pop_store[index]

            # 方法二：如果种群适应度水平小于种群平均适应度水平，则通过种群的选择概率决定是否留下来

            pop_store = pop_store_new
            # 种群进行交配、个体进行变异、倒位
            # 交配过程
            # 确定交配染色体的数量
            if self.pc_num is not None:
                number = int(self.popsize*self.pc_num)
            else:
                number = int(self.popsize * np.random.rand())
            for k in range(number):
                # 方法一：选择概率最小的两个个体进行交配
                # 产生pop_size 的随机数
                probability = np.random.rand(self.popsize)
                ...
                pop_store[index1] = new_itema
                pop_store[index2] = new_itemb
            # 个体的基因突变和倒位
            for k in range(self.popsize):
            ...
            for k in range(self.popsize):
                ...
                length_sum += length
            self.sol_length_avg.append(length_sum/self.popsize)
            print("iter:%d,best:%.4f"%(_iter,self.E_best))
